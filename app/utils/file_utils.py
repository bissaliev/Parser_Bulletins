...
class XLSExtractor:
    ...
    def __init__(self, file: io.BytesIO, bidding_date: date):
        ...
        logger.info(f"Файл загружен для даты {bidding_date}")
    ...
    def _find_start_index(self) -> int:
        ...
        match = (
            self.dataframe.stack().astype(str).str.contains(self.table_name)
        )
    ...
    def _extract_table(self):
        ...
        self.df.columns = (
            self.df.iloc[0].astype(str).str.replace("\n", " ").str.strip()
        )
    ...
    def _filter_valid_rows(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        count_col = next(
            col for col in df.columns if "Количество Договоров" in col
        )
    ...
    def get_data(self) -> list[dict[str, Any]]:
        ...
        try:
            data = self._to_dict(
                self._filter_valid_rows(self._extract_table())
            )
            logger.info(f"Данные успешно обработаны для {self.bidding_date}")
            return data
        except (ValueError, KeyError, StopIteration) as e:
            logger.error(f"Ошибка обработки XLS-файла: {e}")
            raise XLSExtractorError(
                f"Ошибка при обработке XLS-файла: {e}"
            ) from e
